...
class DashboardController extends Controller
{
    ...
    public function getRegistrationStats()
    {
        ...
        $stats = StudentRegistration::selectRaw('MONTH(created_at) as month, COUNT(*) as count')
    ...
    public function getAcademicPerformance()
    {
        $stats = DB::table('grades')
            ...
            ->select('courses.course_name', DB::raw('AVG(grades.score) as average_score'))
    ...
    public function getFinanceStats()
    {
        try {
            ...
            $unpaid = Payment::whereIn('status', ['unpaid', 'overdue'])->count();
    ...
    public function getSummaryStats()
    {
        try {
            ...
            try {
                $pendingRegistrations = StudentRegistration::where('registration_status', 'pending_approval')->count();
            } catch (\Exception $e) {
    ...
    public function getStudentRiskAnalysis()
    {
        try {
            ...
            $enrollments = \App\Models\Enrollment::with(['student.user', 'course', 'grades'])
                ->where('status', 'active')
                ->get();

            foreach ($enrollments as $enrollment) {
                $avgScore = $enrollment->grades->avg('score') ?? 0;
                ...
                try {
                    ...
                    $totalSessions = \App\Models\AttendanceSession::where('course_id', $enrollment->course_id)->count();
                    if ($totalSessions > 0) {
                        $present = \App\Models\AttendanceRecord::where('enrollment_id', $enrollment->id)
                ...
                if ($avgScore < 60 || $attendancePct < 75) {
                    $atRiskStudents[] = [
                        ...
                        'parent_phone' => $enrollment->student->phone_number ?? '-', // Fallback to student phone
                        'risk_factors' => [
                            'grade' => $avgScore < 60,
    ...
    {
        try {
            $totalVerifications = \App\Models\Certificate::sum('verification_count');
            
            // Get top course by verification
            $topCourse = \App\Models\Certificate::select('course_id', DB::raw('SUM(verification_count) as total_verifications'))
                ->groupBy('course_id')
            ...
            return response()->json([
                ...
                'top_course' => $topCourse ? $topCourse->course->course_name : '-'
            ]);
        } catch (\Exception $e) {
            return response()->json([
