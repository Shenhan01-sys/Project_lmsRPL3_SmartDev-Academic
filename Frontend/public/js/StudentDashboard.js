...
function ensureSubmitModalExists() {
    if (!document.getElementById("modal-submit-assignment")) {
        const modalHTML = `
...
async function loadCourses() {
    ...
    // Ensure submit modal exists
    ensureSubmitModalExists();
}
...
function renderCourseModules(modules) {
    ...
    container.innerHTML =
        ...
        <div class="border border-slate-200 rounded-xl overflow-hidden bg-white mb-4">
            <div class="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center cursor-pointer" onclick="document.getElementById('mod-body-${m.id}').classList.toggle('hidden')">
                <h4 class="font-bold text-slate-800">${m.title || m.module_name}</h4>
                <i class="bi bi-chevron-down text-slate-400 text-xs"></i>
            </div>
            <div id="mod-body-${m.id}" class="hidden p-4 space-y-3">
                <p class="text-sm text-slate-600">${m.description || ""}</p>
                <div class="space-y-2 border-t pt-2">
                    ${
                        materials
                            .map((mat) => {
                                // Logic link - sama seperti InstructorDashboard
                                let fullLink = "#";
                        ...
                        <div class="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-lg">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-slate-400 uppercase">${mat.material_type || "file"}</p>
                            </div>
                            ${mat.content_path ? `<a href="${fullLink}" target="_blank" class="px-3 py-1 text-xs font-bold text-primary bg-indigo-50 rounded hover:bg-indigo-100">Buka</a>` : ""}
                        </div>
                    `;
                            })
                            .join("") ||
...
async function renderCourseAssignments(list) {
    ...
    if (!list || list.length === 0) {
        container.innerHTML =
            '<p class="text-center py-4 text-slate-400">Tidak ada tugas.</p>';
        return;
    }
    ...
    const html = list
        .map((assignment) => {
            ...
            if (submission) {
                ...
                // Show last submitted time if available
                if (submission.submitted_at || submission.updated_at) {
                    const lastUpdate = new Date(
                        submission.submitted_at || submission.updated_at,
                    );
                    statusBadge += ` <span class="text-xs px-2 py-1 rounded font-bold bg-slate-100 text-slate-600"><i class="bi bi-clock-history"></i> ${formatDate(lastUpdate)}</span>`;
                }
                ...
                // Button logic based on status and deadline
                if (submission.status === "graded") {
                    // If graded, only show view detail button (no resubmission)
                    ...
                    // Before deadline: allow submission/resubmission
                    const buttonText =
                        submission.status === "draft"
                            ? "Edit & Submit"
                            : "Submit Ulang";
                    actionButton = `<button onclick="openSubmitModal(${assignment.id}, ${submission.id})" class="px-3 py-1 bg-blue-600 text-white rounded text-xs font-bold hover:bg-blue-700"><i class="bi bi-pencil"></i> ${buttonText}</button>`;
                } else {
                    // Past deadline: no more submissions allowed
                    actionButton = `<span class="text-xs text-slate-500"><i class="bi bi-clock-history"></i> Waktu habis - Menunggu penilaian</span>`;
                }
            } else {
                statusBadge =
                    '<span class="text-xs px-2 py-1 rounded font-bold bg-red-100 text-red-700">Belum Dikumpulkan</span>';
            ...
            const now = new Date();
            const isOverdue =
                dueDate < now && (!submission || submission.status === "draft");
            const dueDateColor = isOverdue
                ? "text-red-600 font-bold"
                : "text-slate-600";

            // Show grade prominently if graded
            let gradeDisplay = "";
            if (
                submission &&
                submission.status === "graded" &&
                ...
                gradeDisplay = `<div class="text-right"><div class="text-3xl font-bold text-green-600">${submission.grade}</div><div class="text-xs text-slate-500">dari ${assignment.max_score || 100}</div></div>`;
            }

            return `<div class="dashboard-card p-4 mb-3">
                <div class="flex justify-between items-start mb-3">
                    <div class="flex-1">
                        <h4 class="font-bold text-slate-800 mb-1">${assignment.title}</h4>
                        <p class="text-xs ${dueDateColor} mb-2">
                            <i class="bi bi-clock"></i> Due: ${formatDate(assignment.due_date)}${isOverdue ? '<span class="ml-2 text-red-600">⚠️ OVERDUE</span>' : ""}
                        </p>
                        <p class="text-sm text-slate-600 mb-2">${assignment.description || "Tidak ada deskripsi"}</p>
                    </div>
                    ${gradeDisplay}
                </div>
                <div class="flex justify-between items-center pt-3 border-t border-slate-100">
                    <div class="flex gap-2 items-center flex-wrap">${statusBadge}</div>
                    ${actionButton}
                </div>
            </div>`;
        })
...
async function loadMySubmissions() {
    try {
        // Backend otomatis filter berdasarkan user yang login
        const res = await fetchApi(`/submissions`);
        mySubmissions = Array.isArray(res) ? res : res.data || [];
        return mySubmissions;
    } catch (error) {
        console.error("Error loading submissions:", error);
        // Jika error 500, kembalikan array kosong agar tidak break UI
        mySubmissions = [];
        return [];
    }
}
...
function openSubmitModal(assignmentId, submissionId = null) {
    ...
    if (!assignment) {
        Swal.fire({
            icon: "error",
            title: "Error",
            text: "Assignment tidak ditemukan",
        });
        return;
    }
    ...
    document.getElementById("submission-assignment-title").textContent =
        assignment.title;
    document.getElementById("submission-due-date").textContent = formatDate(
        assignment.due_date,
    );
    document.getElementById("submission-assignment-desc").textContent =
    ...
    if (submissionId) {
        ...
        if (submission && submission.file_path) {
            ...
            document
                .getElementById("current-file-name")
                .classList.remove("hidden");
            document
                .getElementById("submission-file")
...
async function submitAssignment(event, submitNow = true) {
    ...
    const assignmentId = document.getElementById(
    ...
    // Defensive check: pastikan fileInput element ada
    if (!fileInput) {
        await Swal.fire({
            icon: "error",
            title: "Error",
    ...
    try {
        ...
        const targetSubmissionId = submissionId || existingSubmission?.id;
        ...
        if (targetSubmissionId) {
            ...
            response = await fetchApi(`/submissions/${targetSubmissionId}`, {
                method: "POST",
                body: formData,
            });
        } else {
        ...
        const isResubmission = targetSubmissionId ? true : false;
        const statusText = submitNow ? "dikumpulkan" : "disimpan sebagai draft";
        let message = isResubmission
            ? `Tugas berhasil di${statusText} ulang!`
            : `Tugas berhasil ${statusText}!`;
        ...
        if (isResubmission) {
            message += ` File lama telah diganti dengan file baru.`;
        }
...
async function viewSubmissionDetail(submissionId) {
    try {
        ...
        let content = `<div class="text-left space-y-4">
                ...
                ${submission.file_path ? `<div><label class="text-xs font-bold text-slate-500">File</label><a href="${submission.file_path}" target="_blank" class="text-blue-600 hover:underline flex items-center gap-2"><i class="bi bi-download"></i> Download File</a></div>` : ""}
            </div>`;
