    ...
    async function fetchApi(endpoint, options = {}) {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error(
                    json?.message || `HTTP Error ${res.status}`,
                );
    ...
    async function loadDashboardData() {
        ...
        try {
            ...
            const [
                ...
                fetchApi("/courses"),
                fetchApi("/registrations?status=pending").catch(
                    (e) => ({ total: 0 }),
                ),
            ...
            store.courses = Array.isArray(courses)
                ? courses
                : courses?.data || [];
            renderCourses();
    ...
    async function loadRealEWSData() {
        ...
        try {
            ...
            atRisk.forEach((e) => {
                const studentName =
                    e.student?.user?.name || "Siswa Tanpa Nama";
    ...
    async function loadRealScatterData() {
        ...
        try {
            ...
            const enrollmentsRes = await fetchApi(
                "/enrollments?per_page=200",
            );
            ...
            const scatterDataPromises = enrollments.map(
                async (enrollment) => {
                    try {
                        const studentId = enrollment.student_id;
                        const courseId = enrollment.course_id;
                        const finalGrade = parseFloat(
                            enrollment.final_grade || 0,
                        );
            ...
            const scatterDataRaw =
                await Promise.all(scatterDataPromises);

            // Filter null values
            const scatterData = scatterDataRaw.filter(
                (item) => item !== null,
            );

            console.log(
                "✅ Valid scatter data points:",
                scatterData.length,
            );
            console.log(
                "📈 Sample data (first 5):",
                scatterData.slice(0, 5),
            );
            ...
            console.log(
                `   - Without attendance data (x=0): ${withoutAttendance}`,
            );

            // 🔍 Show beberapa contoh yang x=0
            const zeroAttendance = scatterData
                .filter((d) => d.x === 0)
                .slice(0, 3);
                    ...
                    datasets: [
                        {
                            ...
                            borderColor: (ctx) => {
                                const v = ctx.raw;
                                if (!v || !v.hasAttendance)
                                    return "#94a3b8"; // Abu-abu border untuk yang x=0
                                return "transparent";
                            },
                            borderWidth: (ctx) => {
                                const v = ctx.raw;
            ...
            Swal.fire({
                ...
                title: "Error",
                text:
                    "Gagal memuat data scatter plot: " + error.message,
            });
        }
    }
    ...
    function updateCharts() {
        ...
        if (ctxReg && store.stats.registrations) {
            if (Chart.getChart(ctxReg))
                Chart.getChart(ctxReg).destroy();
            new Chart(ctxReg, {
                type: "line",
                data: {
        ...
        if (ctxDist && store.stats.users) {
            if (Chart.getChart(ctxDist))
                Chart.getChart(ctxDist).destroy();
    ...
    function nav(viewId) {
        ...
        if (!targetView) {
            console.error(
                `Element with id "view-${viewId}" not found!`,
            );
            return;
        }
    ...
    async function showRegistrationDetail(id) {
        try {
            ...
            document.getElementById("detail-reg-id").textContent =
                `#${reg.id}`;
            ...
            ).textContent = reg.user?.email || "-";
            document.getElementById("detail-student-role").innerHTML =
                reg.user?.role
            ...
            ).textContent = reg.tanggal_lahir
                ? new Date(reg.tanggal_lahir).toLocaleDateString(
                        "id-ID",
                        {
                            day: "2-digit",
                            month: "long",
            ...
            const gender =
                reg.jenis_kelamin === "L"
                    ...
                    : reg.jenis_kelamin === "P"
                        ...
                        : "-";
            document.getElementById("detail-jenis-kelamin").innerHTML =
                gender;
            ...
            ).textContent = reg.alamat_orang_tua || "-";
            ...
            document.getElementById("detail-approved-at").textContent =
                reg.approved_at
                    ? new Date(reg.approved_at).toLocaleString(
                            "id-ID",
                            {
                                day: "2-digit",
                                month: "short",
            ...
            if (reg.approval_notes) {
                ...
                ).textContent = reg.approval_notes;
            ...
            const actionsContainer =
                document.getElementById("detail-actions");
            if (
    ...
    function updatePagination(totalPages, current) {
        document.getElementById("btn-prev-student").disabled =
            current <= 1;
    ...
    async function submitCreateCourse() {
        const payload = {
            ...
            instructor_id: document.getElementById(
    ...
    async function showInstructorDetail(id) {
        ...
        document.getElementById("det-ins-email").innerText =
            resIns.email;
        document.getElementById("det-ins-spec").innerText =
            resIns.specialization;
        document.getElementById("det-ins-phone").innerText =
            resIns.phone || "-";
        document.getElementById("det-ins-avatar").innerText =
            insName.charAt(0);
        ...
        document.getElementById("det-ins-students").innerText =
            totalStudents;
        document.getElementById("det-ins-courses").innerText =
            myCourses.length;

        document.getElementById("det-ins-course-list").innerHTML =
            myCourses.length
                ? myCourses
                        .map(
                            (c) => `
        ...
        const btnEdit = document.getElementById(
            "btn-edit-instructor-detail",
        );
        if (btnEdit)
            btnEdit.onclick = () => openEditInstructorModal(id);
    ...
    async function openEditInstructorModal(id) {
        ...
        document.getElementById("edit-ins-name").value =
            res.full_name || res.name;
        document.getElementById("edit-ins-email").value = res.email;
        document.getElementById("edit-ins-code").value =
            res.instructor_code || "";
        document.getElementById("edit-ins-phone").value =
            res.phone || "";
        document.getElementById("edit-ins-spec").value =
            res.specialization || "";
        document.getElementById("edit-ins-edu").value =
            res.education_level || "S1";
        document.getElementById("edit-ins-exp").value =
            res.experience_years || 0;
        document.getElementById("edit-ins-bio").value = res.bio || "";
        document.getElementById("edit-ins-status").value =
            res.status || "active";
